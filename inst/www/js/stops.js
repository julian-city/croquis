// Namespace prefix for stops module (set by server)
var stopsNs = '';

Shiny.addCustomMessageHandler('setStopsNs', function(ns) {
  stopsNs = ns;
});

// Stop search input handler
$(document).on('input', '#stop_search', function() {
  Shiny.setInputValue(stopsNs + 'stop_search_term', $(this).val(), {priority: 'event'});
});

// Start editing a stop from the list
function editStopFromList(stopId) {
  Shiny.setInputValue(stopsNs + 'stop_list_edit_click', stopId, {priority: 'event'});
}

// Start adding a new stop
function startAddingStop() {
  Shiny.setInputValue(stopsNs + 'stop_list_add_click', Math.random(), {priority: 'event'});
}
...
function saveEditingStop() {
  ...
  Shiny.setInputValue(stopsNs + 'stop_list_save_data', {
    stop_id: stopId,
    stop_name: stopName,
...
function cancelEditingStop() {
  Shiny.setInputValue(stopsNs + 'stop_list_cancel_click', Math.random(), {priority: 'event'});
}

// View/focus on a stop without editing
function viewStopFromList(stopId) {
  Shiny.setInputValue(stopsNs + 'stop_list_view_click', stopId, {priority: 'event'});
}
